...
@section('content')
    ...
    <p>
        @include('layouts.form_edit_title', ['update'=>$update, 'table_name'=>trans('main.level')])
    </p>
    <form action="{{$update ? route('level.update',$level):route('level.store')}}" method="POST"
            ...
            @foreach (config('app.locales') as $key=>$value)
                ...
                <div class="col-7">
                    <input type="text"
                           ...
                           placeholder=""
                           value="{{ old('name_lang_' . $key) ?? ($level['name_lang_' . $key] ?? '') }}">
                </div>
            ...
            <div class="row text-center">
                ...
                <div class="col-5 text-left">
                    <button type="button" class="btn btn-dreamer" title="{{trans('main.cancel')}}"
                        @include('layouts.level.previous_url')
                    >
